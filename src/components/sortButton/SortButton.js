...
import { refreshState, sortState } from "../../store/tableSlice";

function SortButton({ sortKey = "" }) {
  const dispatch = useDispatch();
  const sortedData = useSelector((state) => state.table.sortedData);
  const sortedKey = useSelector((state) => state.table.sortedKey);
  const [index, setIndex] = useState(0);
  const bool = useRef(false);
  const type = ["none", "up", "down"];

  useEffect(() => {
    if (!sortedData && index > 0) {
      setIndex(0);
    }
  }, [sortedData]);
    ...
    >
      ...
      >
        <path
          ...
          stroke="#e4e4e4"
          fill={
            type[index] == "up" && sortKey == sortedKey ? "#e4e4e4" : "none"
          }
          strokeWidth="2"
          strokeLinecap="round"
        ...
        <path
          ...
          stroke="#e4e4e4"
          fill={
            type[index] == "down" && sortKey == sortedKey ? "#e4e4e4" : "none"
          }
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
